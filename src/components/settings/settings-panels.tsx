        ...
        >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              <Input id="username" value={store.username} onChange={e => store.updateSetting("username", e.target.value)} required className="h-9 text-xs rounded-xl" />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1 sm:col-span-2">
              ...
              <textarea 
                id="bio" 
                value={store.bio} 
                onChange={e => store.updateSetting("bio", e.target.value)} 
            ...
            <div className="space-y-1">
              ...
              <Input id="studentId" value={store.studentId} onChange={e => store.updateSetting("studentId", e.target.value)} className="h-9 text-xs rounded-xl" placeholder="e.g. NR-2026" />
            </div>
            <div className="space-y-1">
              <Label htmlFor="country" className="text-[10px] font-bold text-muted-foreground uppercase">Country</Label>
              <Input id="country" value={store.country} onChange={e => store.updateSetting("country", e.target.value)} className="h-9 text-xs rounded-xl" />
            </div>
            <div className="space-y-1">
              ...
              <select 
                ...
                onChange={e => store.updateSetting("timezone", e.target.value)}
        ...
        <DialogContent className="sm:max-w-md bg-card border border-border/80 p-6 rounded-2xl shadow-2xl">
          ...
          <div className="py-6 flex flex-col items-center text-center space-y-4 bg-muted/20 border border-border/40 rounded-xl p-4 mt-2">
            ...
            <div>
              ...
              <p className="text-[9px] text-primary uppercase font-bold tracking-wider mt-0.5">@{store.username || "student"}</p>
            </div>
            <p className="text-xs text-muted-foreground max-w-xs leading-relaxed italic">
              "{store.bio || "No biography provided yet."}"
            </p>
            <div className="w-full grid grid-cols-2 gap-2 text-left pt-4 border-t border-border/40 text-[10px] text-muted-foreground">
              ...
              <div><span className="font-bold text-foreground">Department:</span> {store.department || "Not set"}</div>
              <div><span className="font-bold text-foreground">Semester:</span> {store.semester || "Not set"}</div>
              <div><span className="font-bold text-foreground">Country:</span> {store.country || "Not set"}</div>
              <div><span className="font-bold text-foreground">Student ID:</span> {store.studentId || "None"}</div>
            </div>
          </div>
          <DialogFooter>
...
export function AIPreferences() {
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <SectionCard 
        title="AI Study Pack Auto-Trigger Settings" 
        description="Choose which uploaded file categories should automatically trigger AI Study Pack generation."
        icon={<BrainCircuit className="h-4 w-4 text-primary" />}
      >
        <div className="divide-y divide-border/40 text-xs">
          <SettingRow label="Auto Generate for Lectures" description="Automatically generate study packs for files classified as lectures.">
            <Switch checked={store.aiAutoLectures} onChange={val => store.updateSetting("aiAutoLectures", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Notes" description="Automatically generate study packs for notes files.">
            <Switch checked={store.aiAutoNotes} onChange={val => store.updateSetting("aiAutoNotes", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Presentations" description="Automatically generate study packs for presentations (slides/pptx).">
            <Switch checked={store.aiAutoPresentations} onChange={val => store.updateSetting("aiAutoPresentations", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Assignments" description="Automatically generate study packs for assignment files.">
            <Switch checked={store.aiAutoAssignments} onChange={val => store.updateSetting("aiAutoAssignments", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Quizzes" description="Automatically generate study packs for quiz files.">
            <Switch checked={store.aiAutoQuizzes} onChange={val => store.updateSetting("aiAutoQuizzes", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Projects" description="Automatically generate study packs for projects.">
            <Switch checked={store.aiAutoProjects} onChange={val => store.updateSetting("aiAutoProjects", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Lab Material" description="Automatically generate study packs for lab manual/task files.">
            <Switch checked={store.aiAutoLabs} onChange={val => store.updateSetting("aiAutoLabs", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Past Papers" description="Automatically generate study packs for exam papers or past papers.">
            <Switch checked={store.aiAutoPastPapers} onChange={val => store.updateSetting("aiAutoPastPapers", val)} />
          </SettingRow>
          <SettingRow label="Auto Generate for Administrative Documents" description="Always disabled. Files classified as challan, receipts, registrations, outlines, circulars.">
...
export function AccountSettings({ onLogoutClick, onDeleteAccountClick, dbStats: _dbStats }: AccountSettingsProps) {
  const store = useSettingsStore();
