...
import { 
  ...
  Shield, Eye, Key, Info, Search, 
  Trash2, CheckCircle2, Menu, X, AlertTriangle, Loader2, ChevronRight
} from "lucide-react";
...
export function ProfileClient({
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (res.success) {
        // Reset client-side settings store
        store.resetAll();
        localStorage.removeItem("neuron-settings-storage");
      ...
      console.error("Account deletion handler caught error:", error);
      setDeleteError(error?.message || "Account deletion failed. Please try again or contact support.");
    } finally {
        ...
        <DialogContent className="sm:max-w-[440px] bg-card border border-border/80 p-6 rounded-2xl shadow-2xl text-left">
          <DialogHeader className="gap-3 border-b border-border pb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-sm font-bold text-foreground">Delete Account</DialogTitle>
                <DialogDescription className="text-[10px] text-destructive/80 font-bold uppercase tracking-wider mt-0.5">This action is permanent and cannot be undone</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-4 my-4">
            <div className="p-4 rounded-xl bg-muted/40 border border-border/60 text-muted-foreground text-xs leading-relaxed">
              <p className="font-bold text-foreground mb-1.5">Deleting your account will permanently remove:</p>
              <ul className="list-disc pl-5 mt-2 space-y-0.5 text-muted-foreground font-medium">
                <li>Subjects</li>
                <li>Folders</li>
                <li>Files</li>
                <li>Uploaded PDFs</li>
                <li>AI Generated resources</li>
                <li>Summaries</li>
                <li>Flashcards</li>
                <li>Quizzes</li>
                <li>Study plans</li>
              ...
              <p className="font-bold text-destructive/90 mt-3">This action cannot be reversed.</p>
            </div>
